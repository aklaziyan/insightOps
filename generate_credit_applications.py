...
portfolio_choices = (["CF"] * 4500 + ["PF"] * 3000 + ["AF"] * 1500 + ["CORP"] * 300 + ["MORTGAGE"] * 700)
...
def np_normal_int(mean, std, low, high):
    x = int(np.clip(np.random.normal(mean, std), low, high))
    return x
...
for i in range(N):
    ...
    if portfolio in ["CF", "PF"]:
        ...
        # Ara değer ekle: ör. 72, 96, 108 ay
...
sql = f"INSERT INTO credit_application ({columns}) VALUES ({', '.join(['%s'] * len(row))})"
